...
/***********************************************
** File name: 488B.cpp                        **
** Time created: Sun Dec 2 2018 13:37pm       **
***********************************************/
...
void Solve() {
	...
	if (n == 4) {
		sort(a.begin(), a.end());
		if (a[0] + a[1] + a[2] + a[3] == 2LL * (a[1] + a[2]) && 2LL * (a[1] + a[2]) == (a[3] - a[0]) * 4) {
			cout << "YES\n"; return;
		}
		cout << "NO\n"; return;
	}
	if (n == 3) {
		for (i64 b=1; b<=1000000; b++) {
			a.pub(b); sort(a.begin(), a.end());
			if (a[0] + a[1] + a[2] + a[3] == 2LL * (a[1] + a[2]) && 2LL * (a[1] + a[2]) == (a[3] - a[0]) * 4) {
				cout << "YES\n" << b << endl; return;
			}
	...
	i64 Mn = min(a[0], a[1]), Mx = max(a[0], a[1]);
	for (i64 x=1; x<=500; x++) {
		for (i64 y=1; y<=500; y++) {
			a.pub(x); a.pub(y); sort(a.begin(), a.end());
			if (a[0] + a[1] + a[2] + a[3] == 2LL * (a[1] + a[2]) && 2LL * (a[1] + a[2]) == (a[3] - a[0]) * 4) {
				cout << "YES\n" << x << endl << y << endl; return;
			}
			a.erase(find(a.begin(), a.end(), x));
			a.erase(find(a.begin(), a.end(), y));
		}
	}
	for (i64 x=501; x<=1000000; x++) {
		i64 Rangex4 = (x + Mx) * 2;
		if (Rangex4 % 4 != 0) continue;
		i64 y = Mn + Rangex4 / 4;
		if (x > y) continue;
		if (x+y+Mn+Mx == Rangex4) {cout << "YES\n" << x << endl << y << endl; return;}
	}
	cout << "NO\n";
}
/*****************************************/
