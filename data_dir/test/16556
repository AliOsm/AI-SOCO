...
void Input() {
	...
	for (auto &Tmp: A) {
		...
		for (auto &z: Tmp) cin >> z;
	}
}

void Solve() {
	i64 ans = 0, cnt = A[0].size();
	i64 Max = *max_element(A[0].begin(), A[0].end());
	for (i64 i=1; i<n; i++) {
		i64 nextMax = *max_element(A[i].begin(), A[i].end());
		if (nextMax == Max) {cnt += A[i].size(); continue;}
		if (nextMax < Max) {ans += A[i].size() * (Max - nextMax); cnt += A[i].size(); continue;}
		if (nextMax > Max) {ans += cnt * (nextMax - Max); cnt += A[i].size(); Max = nextMax; continue;}
	}
	cout << ans << endl;
}
/*****************************************/
