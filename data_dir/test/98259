...
void Solve() {
	vector<vector<vi>> Dist(3, vector<vi>(1001, vi(1001, LINF)));
	for (i64 i=0; i<3; i++) {
		queue<pii> Q; Q.push(A[i]);
		Dist[i][A[i].fi][A[i].se] = 0;
		while (!Q.empty()) {
			pii coor = Q.front(); Q.pop();
			i64 x = coor.fi, y = coor.se;
			for (i64 z=0; z<4; z++) {
				if (x + dx[z] < 0 || x + dx[z] > 1000) continue;
				if (y + dy[z] < 0 || y + dy[z] > 1000) continue;
				if (Dist[i][x+dx[z]][y+dy[z]] != LINF) continue;
				Dist[i][x+dx[z]][y+dy[z]] = Dist[i][x][y] + 1;
				Q.push(mp(x+dx[z], y+dy[z]));
			}
		}
	}
	i64 MinDist = LINF; pii Center = mp(-1LL, -1LL);
	for (i64 i=0; i<=1000; i++) {
		for (i64 j=0; j<=1000; j++) {
			i64 Token = Dist[0][i][j];
			Token += Dist[1][i][j];
			Token += Dist[2][i][j];
			if (Token >= MinDist) continue;
			MinDist = Token; Center = mp(i, j);
		}
	}
	set<pii> Points;
	for (auto Point: A) {
		pii X = Point, Y = Center;
		if (X.fi > Y.fi) swap(X, Y);
		if (X.se <= Y.se) {
			for (i64 i=X.se; i<=Y.se; i++) Points.insert(mp(X.fi, i));
			for (i64 i=X.fi; i<=Y.fi; i++) Points.insert(mp(i, Y.se));
		}
		else {
			for (i64 i=Y.se; i<=X.se; i++) Points.insert(mp(X.fi, i));
			for (i64 i=X.fi; i<=Y.fi; i++) Points.insert(mp(i, Y.se));
		}
	}
