...
zet c(int n, int k) {
    return fact[n] * factback[n - k] * factback[k];
}
...
signed main() {
    FAST; FIXED; RANDOM;
    fact[0] = 1;
    for (int i = 1; i < 2000; ++i) fact[i] = fact[i - 1] * i;
    for (int i = 0; i < 2000; ++i) factback[i] = 1 / fact[i];
    int n, m;
    cin >> n >> m;
    zet ans = 0;
    for (int la = 1; la <= n; ++la)
        for (int lb = la; lb <= n; ++lb)
            ans += cc(la, m - 1) * cc(n - lb + 1, m - 1);
    cout << ans;
    return 0;
}