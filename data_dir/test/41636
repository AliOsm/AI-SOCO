...
void f(int x) {
	...
	for (auto &y:g[x]) f(y);
	mint a[2][3]; memset(a,0,sizeof a);
	int cur=0,nxt=1;
	a[cur][0]=1;
	for (auto &y:g[x]) {
		memset(a[nxt],0,sizeof a[nxt]);
		for (int c=0;c<3;c++) {
			a[nxt][c]+=a[cur][c]*(dp[y][0]+dp[y][2]); // Different color
			a[nxt][min(2,c+1)]+=a[cur][c]*(dp[y][1]+dp[y][2]); // Same color
		}
		swap(cur,nxt);
	}
	for (int c=0;c<3;c++) {
...
int main() 
{
  ...
  int n; cin>>n; for (int i=2;i<=n;i++) {int x; cin>>x; g[x].PB(i);}
  f(1);
