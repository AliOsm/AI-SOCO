...
int main(int argc, char** argv) {
    ...
    map<pair<llong, llong>, llong> m[63];

    function<llong(llong, llong, int)> go = [&](llong s, llong x, int t) -> llong {
        if (s < 0) {
            return 0LL;
        }
        if (t < 0) {
        ...
        if ((x >> t) & 1) {
            return m[t][{s, x}] = 2 * go(s - (1LL << t), x, t - 1);
        }

        return m[t][{s, x}] = go(s, x, t - 1) + go(s - (1LL << (t + 1)), x, t - 1);
    };
