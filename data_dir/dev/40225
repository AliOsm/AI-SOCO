...
int Rank[N / 10][22], Mc[N / 10][22], stp, n, k;
char in[N];


void SuffixArray(){
    stp = 1;
    Rep(i, n) Rank[0][i] = in[i] - 'a';
    for(int sz = 1; sz < n; sz *= 2, stp++) {
        ...
        int rk = 0;
        For(i, 1, n) {
            if(sfx[i - 1].f != sfx[i].f || sfx[i - 1].s != sfx[i].s) rk++;
...
int main()
{
    ...
    Rep(i, n) {
        ...
        for(int sz = lcp[i] + 1; st + sz <= n; sz++) {
            ...
            if(k == 0) {
                For(j, st, st + sz) printf("%c", in[j]); pc('\n');
                return 0;
            }
        }
    }
