...
/***********************************************
** File name: 615C.cpp                        **
** Time created: Fri Dec 21 2018 11:59am      **
***********************************************/
...
void Solve() {
	vector<vi> lcp_s(m, vi(n, 0));
	vector<vi> lcp_rs(m, vi(n, 0));
	for (i64 i=m-1; i>=0; i--) {
		for (i64 j=n-1; j>=0; j--) {
			if (t[i] == s[j]) {
				if (i == m-1 || j == n-1) lcp_s[i][j] = 1;
				else lcp_s[i][j] = lcp_s[i+1][j+1] + 1;
			}
			if (t[i] == rs[j]) {
				if (i == m-1 || j == n-1) lcp_rs[i][j] = 1;
				else lcp_rs[i][j] = lcp_rs[i+1][j+1] + 1;
			}
		}
	}
	vector<pii> Coating; i64 id = 0;
	while (id < m) {
		i64 Max = 0; pii Token = mp(-1LL, -1LL);
		for (i64 i=0; i<n; i++) {
			if (lcp_s[id][i] <= Max) continue;
			Max = lcp_s[id][i];
			Token = mp(i + 1, i + Max - 1 + 1);
		}
		for (i64 i=0; i<n; i++) {
			if (lcp_rs[id][i] <= Max) continue;
			Max = lcp_rs[id][i];
			Token = mp((n - 1) - i + 1, (n - 1) - (i + Max - 1) + 1);
		}
		if (Max == 0) {cout << "-1\n"; return;}
		id += Max; Coating.pub(Token);
	}
	cout << Coating.size() << endl;
	for (auto z: Coating) cout << z.fi << " " << z.se << endl;
}
/*****************************************/
