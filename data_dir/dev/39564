...
/***********************************************
** File name: 1082B.cpp                       **
** Time created: Sat Dec 1 2018 07:03am       **
***********************************************/
...
void Solve() {
	...
	for (i64 i=0; i<n; i++) {
		if (s[i] == 'S') continue;
		i64 j = i;
		while (j+1 < n && s[j+1] == 'G') j++;
		Streak.pub(mp(i, j)); i = j;
	}
	if (Streak.empty()) {cout << "0\n"; return;}
	if (Streak.size() == 1) {cout << Streak[0].se - Streak[0].fi + 1; return;}
	i64 ans = 0;
	for (i64 i=0; i<Streak.size(); i++) {
		ans = max(Streak[i].se - Streak[i].fi + 2, ans);
	}
	for (i64 i=1; i<Streak.size(); i++) {
		if (Streak[i].fi - Streak[i-1].se > 2) continue;
		ans = max(ans, Streak[i].se - Streak[i].fi + 1 + Streak[i-1].se - Streak[i-1].fi + 1 + (Streak.size() > 2));
	}
	cout << ans << endl;
}
/*****************************************/
