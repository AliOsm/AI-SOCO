...
/************** [GLOBAL VARIABLES] **************/
int n, q; vl a, k, psk, pswk, Tree; vector<pll> Lazy;
pll Default = mp(-LINF, +LINF);
/************************************************/

/************** [FUNCTIONS] **************/
i64 getKRange(int L, int R) {
	i64 res = psk[R];
	if (L > 0) res -= psk[L-1];
	return res;
}

i64 getKWRange(int L, int R) {
	i64 res = pswk[R];
	if (L > 0) res -= pswk[L-1];
	res -= getKRange(L, R) * (n-2 - R);
	return res;
}

void Propagate(int node, int st, int en) {
	if (Lazy[node] == Default) return;
	i64 val = Lazy[node].fi, id = Lazy[node].se;
	Tree[node] = (val + ((id == st-1) ? 0 : getKRange(id, st-2))) * (en - st + 1);
	Tree[node] += getKWRange(st-1, en-1);
	if (st != en) {
...
void Update(int node, int st, int en, int id, int val) {
	...
	if (st == id && id == en) {Tree[node] += val; return;}
	Update(node*2+1, st, (st+en)/2+0, id, val);
	Update(node*2+2, (st+en)/2+1, en, id, val);
	Tree[node] = Tree[node*2+1] + Tree[node*2+2];
}

void Fill(int node, int st, int en, int L, int R, i64 val, int id) {
	Propagate(node, st, en);
	if (st > en || en < L || R < st) return;
	if (L <= st && en <= R) {
		Lazy[node] = mp(val, 1LL*id);
		Propagate(node, st, en); return;
	}
	Fill(node*2+1, st, (st+en)/2+0, L, R, val, id);
	Fill(node*2+2, (st+en)/2+1, en, L, R, val, id);
	Tree[node] = Tree[node*2+1] + Tree[node*2+2];
}
...
i64 binsearch(i64 val, i64 top, i64 bot) {
	i64 res = top - 1, head = top-1;
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		if (val + getKRange(head, mid-1) > Get(0, 0, n-1, mid, mid)) {
			res = mid; top = mid + 1;
		}
		else bot = mid - 1;
	}
...
void Input() {
	cin >> n; a.rsz(n); k.rsz(n-1);
	Tree.rsz(n*4); Lazy.rsz(n*4, Default);
	for (auto &z: a) cin >> z;
	for (auto &z: k) cin >> z;
}

void Solve() {
	psk.rsz(n-1, k[0]); pswk.rsz(n-1, k[0]*(n-1));
	for (int i=1; i<n-1; i++) psk[i] = psk[i-1] + k[i];
	for (int i=1; i<n-1; i++) pswk[i] = pswk[i-1] + k[i] * (n-1 - i);
	for (int i=0; i<n; i++) Update(0, 0, n-1, i, a[i]);
	cin >> q;
	while (q--) {
		char t; int i, x, l, r;
		cin >> t >> i >> x;
		if (t == '+') {i--;}
		else if (t == 's') {l = i-1; r = x-1;}
		if (t == '+') {
			Update(0, 0, n-1, i, x);
			i64 val = Get(0, 0, n-1, i, i);
			i64 LastFill = binsearch(val, i+1, n-1);
			if (LastFill > i) Fill(0, 0, n-1, i+1, LastFill, val, i);
		}
		else if (t == 's') {
