...
/***********************************************
** File name: CFR529D3F.cpp                   **
** Time created: Thu Dec 27 2018 14:42pm      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, x, y, w; vi a; multiset<i64> MinMS;
vi DSUsize, DSUpar;
/************************************************/
...
void Solve() {
	...
	i64 ans = 0, id = min_element(a.begin(), a.end()) - a.begin();
	for (i64 i=0; i<n; i++) if (i != id) Heap.push(mp(a[i]+a[id], mp(id, i)));
	while (m--) {
		cin >> x >> y >> w; x--; y--;
		Heap.push(mp(w, mp(x, y)));
	}
	while (!Heap.empty()) {
		pip Token = Heap.top(); Heap.pop();
		w = Token.fi; x = Token.se.fi; y = Token.se.se;
		x = DSUfind(x); y = DSUfind(y);
		if (x == y) continue;
		if (DSUsize[x] < DSUsize[y]) swap(x, y);
		DSUsize[x] += DSUsize[y]; DSUpar[y] = x; ans += w;
	}
	cout << ans << endl;
}
/*****************************************/
