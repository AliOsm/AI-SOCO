...
LL in[N], n, k, sm[N], sg[N], id[N], ans, a, b;

int main()
{
    scani2(n, k);
    For(i, 1, n + 1) scani(in[i]);
    For(i, 1, n + 1) sm[i] = sm[i - 1] + in[i];
    For(i, 1, n + 1) sg[i] = sm[i + k - 1] - sm[i - 1];
    rFor(i, n, 1) {
        if(sg[i] < sg[i + 1]) {
            sg[i] = sg[i + 1];
            id[i] = id[i + 1];
    ...
    For(i, 1, n) if(ans < sm[i + k - 1] - sm[i - 1] + sg[i + k]) {
        ans = sm[i + k - 1] - sm[i - 1] + sg[i + k];
        a = i;
        b = id[i + k];
    }
    write(a, ' '); write(b);
