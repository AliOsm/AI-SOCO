...
/***********************************************
** File name: 149B.cpp                        **
** Time created: Tue Dec 25 2018 14:09pm      **
***********************************************/
...
void Solve() {
	...
	for (auto c: s) {
		...
		if (!passed) Hr += c; else Mn += c;
	}
	char Max = max(*max_element(Hr.begin(), Hr.end()), *max_element(Mn.begin(), Mn.end()));
	i64 init = Max - '0'; if (Max >= 'A') init = (Max - 'A') + 10;
	for (i64 i=init+1; i<=64; i++) {
		i64 Hour = 0, Min = 0;
		for (auto c: Hr) {
			Hour *= i;
			if (c <= '9') Hour += (c - '0');
		...
		if (Hour < 24 && Min < 60) List.pub(i);
	}
	if (List.empty()) {cout << "0\n"; return;}
	if (List.back() == 64) {cout << "-1\n"; return;}
