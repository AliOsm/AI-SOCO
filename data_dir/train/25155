...
/************** [GLOBAL VARIABLES] **************/
i64 n, k; map<i64, i64> SumDiv, CntDiv;
vi DivList, SD, CD;
/************************************************/
...
void Solve() {
	...
	for (i64 i=2; i<=sqrt(n); i++) {
		...
		if (cnt == 0) continue;
		i64 subans = 0;
		vector<vi> dp(cnt+1, vi(k+1, 0));
		dp[cnt][0] = 1;
		for (i64 x=0; x<k; x++) {
			for (i64 y=0; y<=cnt; y++) {
				dp[0][x+1] += dp[y][x] * modPow(y+1, Mod-2); dp[0][x+1] %= Mod;
				if (y < cnt) {dp[y+1][x+1] -= dp[y][x] * modPow(y+1, Mod-2); dp[y+1][x+1] %= Mod; while (dp[y+1][x+1] < 0) dp[y+1][x+1] += Mod;}
			}
			for (i64 y=1; y<=cnt; y++) {dp[y][x+1] += dp[y-1][x+1]; dp[y][x+1] %= Mod;}
		}
		for (i64 x=0; x<=cnt; x++) {
			subans += modPow(i, x) * dp[x][k];
			subans %= Mod;
		}
		ans *= subans; ans %= Mod;
	}
	if (n > 1) {
		i64 subans = 0, cnt = 1, i = n;
		vector<vi> dp(cnt+1, vi(k+1, 0));
		dp[cnt][0] = 1;
		for (i64 x=0; x<k; x++) {
			for (i64 y=0; y<=cnt; y++) {
