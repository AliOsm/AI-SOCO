...
int main(){
    int n; cin>>n;
    string sr; cin>>sr; for(char c:sr){
        ++cnt[c];
    }
    queue<char> s,d;
    for(int i=0;i<333;++i){
        while(cnt[i]>1)cnt[i]-=2,d.push(i);
        if(cnt[i]>0)s.push(i);
    }
    if(s.empty()){
        stack<char> st;
        cout<<1<<endl;
        while(d.size())cout<<d.front(),st.push(d.front()),d.pop();
        while(st.size())cout<<st.top(),st.pop();
        exit(0);
    }
    for(int i=s.size();i<=n;i+=2){
        if((d.size()-(i-s.size())/2)%i==0){
            stack<char> st;
            int every=(d.size()-(i-s.size())/2)/i;
            cout<<i<<endl;
            for(int iii=0;iii<i;++iii){
            for(int i=0;i<every;++i){
                st.push(d.front()); cout<<d.front(); d.pop();
            }
            if(s.empty())s.push(d.front()),s.push(d.front()),d.pop();
            cout<<s.front(); s.pop();
            while(st.size()){
                cout<<st.top(); st.pop();
            } cout<<" "; }
