...
int main(int argc, char** argv) {
    ...
    n = isz(s);
    vector<pair<llong, int>> es;
    for (int i = 1; i < n; i++) {
        es.emplace_back(s[i] - s[i - 1] - 1, -2);
    }
    int q;
    cin >> q;
    vector<pair<llong, llong>> qs(q);
    for (auto& i : qs) {
        cin >> i.first >> i.second;
        // es.emplace_back(i.second, -1);
        // if (i.first) {
            // es.emplace_back(i.first - 1, -1);
        // }
        es.emplace_back(i.second - i.first, -1);
    }
