...
int main() {
    ...
    while(q--) {
        ...
        if(type == 1) {
            ...
            --x;
            cur -= ll(unread[x].size());
            unread[x].clear();
        } else {
            while(!events.empty() && events.front().ff <= x) {
                pll e = events.front(); events.pop();
                cur -= unread[e.ss].count(e.ff);
                unread[e.ss].erase(e.ff);
            }
        }
        printf("%d\n", cur);
    }
