...
/***********************************************
** File name: 734E.cpp                        **
** Time created: Sat Feb 2 2019 15:30pm       **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi col; vector<vi> adj;
/************************************************/

/************** [FUNCTIONS] **************/
i64 findRoot() {
	vi d(n, 0);
	function<void(i64, i64, i64)> DFS = [&](i64 z, i64 last, i64 depth) -> void {
		d[z] = depth;
		for (auto t: adj[z]) {
			if (t == last) continue;
			DFS(t, z, depth + (col[z] != col[t]));
		}
	};
	DFS(0, -1, 0);
...
void Solve() {
	i64 R = findRoot();
	function<i64(i64, i64, i64)> DFS = [&](i64 z, i64 last, i64 depth) -> i64 {
		i64 val = depth;
		for (auto t: adj[z]) {
			if (t == last) continue;
			val = max(val, DFS(t, z, depth + (col[z] != col[t])));
		}
		return val;
	};
	i64 Diameter = DFS(R, -1, 0);
