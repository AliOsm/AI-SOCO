...
/************** [GLOBAL VARIABLES] **************/
int n, m, D; set<pii> Edge; vector<vi> adj;
vb rootneighbor;
vi DSUpar, DSUsize;
/************************************************/
...
void DFS(int z, vector<vi> &cluster, int clusterID, vb &vis) {
	vis[z] = true;
	if (rootneighbor[z]) cluster[clusterID].pub(z);
	for (auto t: adj[z]) {
		if (vis[t]) continue;
		if (t == 0) continue;
		DFS(t, cluster, clusterID, vis);
	}
}

bool clustersort(vi A, vi B) {
...
void Solve() {
	if (adj[0].size() < D) {cout << "NO\n"; return;}
	vector<vi> cluster; i64 clusterID = 0; vb vis(n, false);
	for (int i=1; i<n; i++) {
		if (vis[i]) continue;
		cluster.pub(vi(0));
		DFS(i, cluster, clusterID, vis);
		clusterID += 1;
	}
	sort(cluster.begin(), cluster.end(), clustersort);
	int curDegree = cluster.size();
	vector<pii> SpanningTree;
	for (auto Cl: cluster) {
		SpanningTree.pub({0, Cl[0]});
		DSUmerge(0, Cl[0]);
		for (int i=1; i<Cl.size(); i++) {
			if (curDegree == D) break;
			curDegree += 1;
			SpanningTree.pub({0, Cl[i]});
			DSUmerge(0, Cl[i]);
		}
	}
	if (curDegree != D) {cout << "NO\n"; return;}
	while (!Edge.empty() && SpanningTree.size() < n-1) {
		pii E = *Edge.begin(); Edge.erase(E);
		int x = E.fi, y = E.se;
		if (DSUfind(x) == DSUfind(y)) continue;
		SpanningTree.pub(E); DSUmerge(x, y);
	}
	if (SpanningTree.size() < n-1) {cout << "NO\n"; return;}
	cout << "YES\n";
	for (auto E: SpanningTree) cout << E.fi+1 << " " << E.se+1 << endl;
}
/*****************************************/
