...
/***********************************************
** File name: 721D.cpp                        **
** Time created: Tue Dec 25 2018 11:20am      **
***********************************************/
...
void Solve() {
	...
	priority_queue<i64, vi, greater<i64>> ZeroHeap;
	i64 Negative = 0, Positive = 0, Zero = 0;
	for (i64 i=0; i<n; i++) {
		if (a[i] == 0) {Zero++; ZeroHeap.push(i); continue;}
		if (a[i] < 0) {Negative++; Heap.push(mp(mp(-a[i], -1LL), i));}
		else {Positive++; Heap.push(mp(mp(+a[i], +1LL), i));}
	}
	if (Zero > 0) {
		if (Zero > k) {for (auto z: a) cout << z << " "; cout << endl; return;}
		if (Negative % 2 == 0) {
			i64 id = ZeroHeap.top(); ZeroHeap.pop(); k--;
			a[id] = -x; Negative++; Heap.push(mp(mp(x, -1LL), id));
		}
		while (!ZeroHeap.empty()) {
			i64 id = ZeroHeap.top(); ZeroHeap.pop(); k--;
			a[id] = x; Positive++; Heap.push(mp(mp(x, +1LL), id));
		}
	}
	if (Negative % 2 == 0) {
		i64 Abs = Heap.top().fi.fi;
		if (Abs >= k * x) {
			ppi Token = Heap.top(); Heap.pop();
			i64 id = Token.se; Token.fi.fi -= k * x;
			a[id] = Token.fi.fi * Token.fi.se;
			for (auto z: a) cout << z << " "; cout << endl; return;
		}
		ppi Token = Heap.top(); Heap.pop();
		i64 demanding = Token.fi.fi / x + 1;
		i64 id = Token.se; Token.fi.fi -= x * demanding;
		Token.fi.fi *= -1; Token.fi.se *= -1; k -= demanding;
		a[id] = Token.fi.fi * Token.fi.se; Heap.push(Token);
	}
	while (k > 0) {
		ppi Token = Heap.top(); Heap.pop();
		i64 id = Token.se; Token.fi.fi += x; k--;
		a[id] = Token.fi.fi * Token.fi.se; Heap.push(Token);
	}
