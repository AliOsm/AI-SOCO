...
void solve() {
    ...
    auto d = make_vector<int, int(1e9)>(n + 1, m + 1, mb + 1);
    vector<vector<vector<tuple<int, int, int, char>>>> p(n + 1, vector<vector<tuple<int, int, int, char>>>(m + 1, vector<tuple<int, int, int, char>>(mb + 1)));
    d[0][0][0] = 1;
    queue<tuple<int, int, int>> q;
    q.emplace(0, 0, 0);

    auto tr = [&](int i, int j, int b, int ni, int nj, int nb) {
        if (d[ni][nj][nb] > d[i][j][b] + 1) {
            d[ni][nj][nb] = d[i][j][b] + 1;
            p[ni][nj][nb] = {i, j, b, (nb > b ? '(' : ')')};
            q.emplace(ni, nj, nb);
        }
    };
