...
/***********************************************
** File name: 442A.cpp                        **
** Time created: Mon Feb 25 2019 11:24am      **
***********************************************/
...
void Input() {
	...
	Map['1'] = 5; Map['2'] = 6; Map['3'] = 7; Map['4'] = 8; Map['5'] = 9;
	cin >> n; a.rsz(n);
	for (auto &s: a) cin >> s;
}
...
void Solve() {
	i64 ans = 10;
	for (i64 msk=0; msk<1024; msk++) {
		vector<set<string>> PartitionList(1);
		for (auto s: a) PartitionList[0].insert(s);
		for (i64 i=0; i<10; i++) {
			if (!getbit(msk, i)) continue;
			i64 n = PartitionList.size();
			for (i64 id=0; id<n; id++) {
				set<string> NewPartition;
				for (auto it=PartitionList[id].begin(); it!=PartitionList[id].end(); it++) {
					string str = *it; if (Map[str[i / 5]] != i) continue;
					NewPartition.insert(str);
				}
				if (NewPartition == PartitionList[id] || NewPartition.empty()) continue;
				PartitionList.pub(NewPartition);
				for (auto it=NewPartition.begin(); it!=NewPartition.end(); it++) {
					string str = *it; PartitionList[id].erase(str);
				}
			}
		}
		bool valid = true;
		for (auto Ptt: PartitionList) {
			if (Ptt.size() > 1) {valid = false; break;}
		}
		if (valid) ans = min(ans, (i64)cntbit(msk));
	}
	cout << ans << endl;
}
/*****************************************/
