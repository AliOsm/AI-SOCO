...
void solve() {
    ...
    auto up = make_vector<int, -1>(n, 22);
    int timer = 1;
    ...
    auto is_anc = [&](int anc, int ch) {
        return tin[anc] <= tin[ch] && tout[anc] >= tout[ch];
    };
    ...
    for (int iq = 0; iq < q; iq++) {
        ...
        cin >> x >> y >> a >> b >> k;
        --x; --y; --a; --b;
        vector<int> ds = {dist(a, b), dist(a, x) + dist(y, b) + 1, dist(a, y) + dist(x, b) + 1};
        bool ok = false;
        for (int d : ds) {
            if (d <= k && (k - d) % 2 == 0) {
                ok = true;
