...
string hou[1111];
bool unk[66],reved[333];
int cnt[333],nad[333],cnas;
int main(){
    int n; cin>>n;
    string s; cin>>s; s="!"+s;
    for(int i=1;i<=n;++i){
    ...
    int m; cin>>m; for(int i=0;i<m;++i){
        cin>>hou[i]; hou[i]="!"+hou[i];
        MS0(nad);
        int tna=0;
        for(int j=1;j<=n;++j){
            if(unk[j] && reved[hou[i][j]]){tna=1; break;}
            else if(unk[j])++nad[hou[i][j]];
            else if(hou[i][j]!=s[j]){tna=1; break;}
        }
        if(tna)++cnas;
        else for(int i=0;i<333;++i)cnt[i]+=!!nad[i];
    }
    int ans=0;
    for(int i=0;i<333;++i)if(cnt[i]==m-cnas)++ans; cout<<ans<<endl;
}
